...
+function ($) {
  ...
  var old = $.fn.tooltipbootstrap

  $.fn.tooltipbootstrap             = Plugin
  $.fn.tooltipbootstrap.Constructor = Tooltip


  // TOOLTIP NO CONFLICT
  // ===================

  $.fn.tooltipbootstrap.noConflict = function () {
    $.fn.tooltipbootstrap = old
    return this
  }
...
+function ($) {
  ...
  Popover.DEFAULTS = $.extend({}, $.fn.tooltipbootstrap.Constructor.DEFAULTS, {
    placement: 'right',
    trigger: 'click',
    content: '',
  ...
  Popover.prototype = $.extend({}, $.fn.tooltipbootstrap.Constructor.prototype)
  ...
  Tour = (function() {
    function Tour(options) {
      ...
      this._options = $.extend({
        ...
        template: '<div class="popover"> <div class="arrow"></div> <h3 class="popover-title gob"></h3> <div class="popover-content"></div> <div class="popover-navigation"> <button class="btn btn-floating" data-role="prev"><i class="fa fa-chevron-circle-left" aria-hidden="true"></i></button> <button class="btn btn-floating" data-role="next"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i></button> <button class="btn btn-floating" data-role="pause-resume" data-pause-text="Pause" data-resume-text="Resume"><i class="fa fa-pause-circle" aria-hidden="true"></i></button> <button class="btn btn-floating" data-role="end"><i class="fa fa-close" aria-hidden="true"></i></button> </div> </div>',
    ...
    Tour.prototype._showBackground = function(step, data) {
      var $backdrop, height, pos, width, _base, _i, _len, _ref, _results, margin;
      margin = 0;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        switch (pos) {
          case 'top':
            _results.push($backdrop.height(data.offset.top > 0 ? data.offset.top - margin: 0 - margin).width(width).offset({
              top: 0,
              left: 0
            }));
            break;
          case 'bottom':
            _results.push($backdrop.offset({
              top: data.offset.top + data.height + margin,
              left: 0
            }).height(height - (data.offset.top + data.height + margin)).width(width));
            break;
          case 'left':
            _results.push($backdrop.offset({
              top: data.offset.top - margin,
              left: 0
            }).height(data.height + margin * 2).width(data.offset.left > 0 ? data.offset.left - margin : 0 - margin));
            break;
          case 'right':
            _results.push($backdrop.offset({
              top: data.offset.top - margin,
              left: data.offset.left + data.width + margin
            }).height(data.height + margin * 2).width(width - (data.offset.left + data.width)));
            break;
          default:
