...
//PLANTAS PARA EMPRESA Y PAIS
function autorizacionesRetornaPlantasPosibles($idEmpresa = 0, $idPais = 0, $idUsuario = 0){
	$db = new SQL2K();
	$params = array(	
        '@IdEmpresa' => array(
			'is_out' => false,
	...
	$res = $db->execute('AutorizacionesRetornaPlantasPosibles',$params);
	$result = $db->fetchAllRow($res);
	$db->disconnect();
	return $result;
}
...
function autorizacionesBuscarDatos($numAutorizacion = 'null', $empresaStr = 'null', $anio = 'null', $estado = 'null', $idPais = 'null', $numRef = 'null', $idUsuario = 0){
	...
	$params = array(
		...
		//esMuestra = 0 para diferenciar de Muestras
        '@esMuestra' => array(
			'is_out' => false,            
            'value' => 0,
            'type' => SQLSRV_PHPTYPE_INT
...
function viasTransporteRetornaLimite($idTransporte = 0){
	$esMuestra = 0; // no es muestra, es AUTORIZACIÓN
	$db = new SQL2K();	
	$params = array(
		'@IdTransporte' => array(
			'is_out' => false,
...
function imprimirCambioAutorizacion($idAut, $idCambio){
	...
	$codeBar = "../classes/barcode/AUT".$_SESSION['id_usuario']."-".$datos['Numero'].".gif";
	$anio = substr($datos['Fecha'],-4);
	$idCode = zerofill($datos['Numero'], 8);
	$code = "AUT".$anio.$idCode;	//lo usa sample-gd
	require("../classes/barcode/sample-gd.php");
	$codigo = "";
	$codeBar = str_replace('..','',$codeBar);
	require("../archivosPDF/codigoAutorizacionCambio.php");	
	$codigoListo = cargarCodigoHTML($codigo,"estiloPDFAutorizacionCambio");//<-- COPIAS
...
function imprimirAutorizacion($idAutorizacion){		
	...
	$codigoListo = cargarCodigoHTML($ret['codigo'],"estiloPDFAutorizacion");//<-- COPIAS
	exportarAPDFOficio($codigoListo, "Solicitud de Exportacion",$ret['codeBar'], traerCuve($idAutorizacion,'AUTORIZACIONES'));
	exit();	
}
...
function codigoPDFExportarAut($idAutorizacion){
	...
	$codigo = "";
        $codeBar = str_replace('..','',$codeBar);
	require("../archivosPDF/codigoAutorizacion.php");
  //agregar DDJJ MIEL
...
function imprimirAutorizacionProrrogada($idAutorizacion){
	...
	$codigoListo = cargarCodigoHTML($ret['codigo'],"estiloPDFAutorizacionProrroga");
...
function codigoPDFProrroga($idAutorizacion){
	...
	$codigo = "";
        $codeBar = str_replace('..','',$codeBar);
	require("../archivosPDF/codigoAutorizacionProrrogada.php");	
...
function imprimirProvisorio($idAutorizacion,$idPlanta,$idPais,$esConsumoAnimal,$esMuestra){
	...
	require("../classes/barcode/sample-gd.php");
	$codeBar = str_replace('..','',$codeBar);
	imprimirDeModulo($codigo,0,0, $codeBar, "Provisorio $idAutorizacion-$idPlanta",false);
}
