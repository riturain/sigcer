...
if (isset($_POST['accion'])){
	...
	case 'traerPlantasDeEmpresa':
		$plantas = muestrasRetornaPlantasPosibles($_POST['id_empresa'], $_SESSION['id_usuario']);
	...
	case 'guardarAutorizacion'://USA MODELO COMÚN				
		//if (validar parámetros){
			...
			$ret = autorizacionesLacteosAbm($_POST['idAutorizacion'],$_POST['nroAutorizacion'],$_POST['fechaAutorizacion'],$_POST['estadoAutorizacion'],$idElaborador,$_POST['idEmpresaExportador'],$_POST['fechaInspeccion'],$_POST['idLugarInspeccion'],utf8_decode($_POST['dirInspeccion']),utf8_decode($_POST['dirVerificacion']),$_POST['idPais'],$_POST['idTransporte'],$_POST['idAduana'],$_POST['idPaso'],$_POST['idDespachante'],$_POST['prorrogarAutorizacion'],$_SESSION['id_usuario'],$observada,$_POST['numOrdenRefAutorizacion'],0,$_POST['copias'],$_POST['requeridos'],$_POST['consumoAnimal'],$_POST['urgencia']);
			//arancel es = 0 porque es muestra (cero)
			if ($_POST['idAutorizacion'] != 0){
				//es una modificación
			...
			if ($ret > 0){				
				...
				switch ($ret){
					case -1:
						...
						break;				
					case -3:
						$_SESSION['rojo'] = "No se han podido guardar los datos, debido a que la muestra tiene un proceso pendiente";
						break;
					case -5:
						$_SESSION['rojo'] = "El número que desea dar de alta ya existe para el año de la muestra";
						break;
				}
			}		
			header('Location: lacteos-muestras.php');
		break;
	...
	*/
	...
	/* llamar AutorizacionesLacteosRetInspeccionesParaCertificados  */
		...
		switch ($ret){
			...
			case '2':
				...
				break;
			case '3':
				echo "3**La empresa no se encuentra habilitada, o alguna de las plantas afectadas no est&aacute; habilitada, o hay operaciones pendientes vinculadas con la muestra.";
				break;
			case '4':
				...
				break;
			case '5':
				echo "5**Desea realizar el certificado sin haber realizado las inspecciones previamente? No podra realizarlas luego.";
				break;
			case '6':
				echo "6**El estado del tr&aacute;mite no permite realizar el certificado sanitario";
		}
		break;
	...
	case 'irACertificadosSinInspecciones':
		$_SESSION['idAut'] = $_POST['idAutorizacion'];
		$_SESSION['inspecciones'] = '#';//$_POST['inspecciones']; //#id#id#..
		echo "1";
		break;
	...
	/* buscador */
		...
		$estados = admRetornaEstadosProcesados("MUESTRAS");
		$presentacionMiel = presentacionesMielRetornaTipos();
	/* fin buscador */
	switch ($_SESSION['perfil']){
			case 2:
				$_SESSION['ruta'] = "Lácteos | Muestras";
				break;
			case '10':
				$_SESSION['ruta'] = "Apícola | Muestras";
				break;
			case 11:
				$_SESSION['ruta'] = "Lácteos/Apícola | Muestras";
				break;
			default:
				$_SESSION['ruta'] = "Muestras sin Valor Comercial";
				break;
	}
	$contenido = 'lacteos-muestras.php';
